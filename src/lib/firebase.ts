...
class FirebaseNotificationService {
  ...
  /**
   * Récupère la configuration Firebase depuis l'Edge Function ou utilise la configuration par défaut
   */
  private async loadFirebaseConfig(): Promise<any> {
    // Vérifier si nous sommes sur l'instance de développement
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || 'https://tornfqtvnzkgnwfudxdb.supabase.co';
    const isDevelopmentInstance = supabaseUrl.includes('whxkhrtlccxubvjgexmi');
    ...
    if (isDevelopmentInstance) {
      ...
      console.log('[FIREBASE DEBUG] Configuration Firebase locale appliquée');
    ...
    try {
      ...
      if (data.config && data.config.apiKey) {
        ...
        console.log('[FIREBASE DEBUG] Configuration Edge Function appliquée');
      } else {
      ...
      console.log('[FIREBASE DEBUG] Utilisation de la configuration par défaut');
    }
    ...
    this.initPromise = new Promise<boolean>(async (resolve) => {
      try {
        ...
        if (!config || !config.apiKey) {
          console.log('[FIREBASE DEBUG] Configuration Firebase invalide, arrêt de l\'initialisation');
          resolve(false);
          return;
        }
