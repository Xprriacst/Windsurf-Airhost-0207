...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    // Rafraîchissement automatique toutes les 5 secondes pour forcer l'affichage
  ...
  const checkSession = async () => {
    ...
    // Permettre l'accès même sans session pour la démonstration des conversations WhatsApp
    // if (!session) {
    //   navigate('/login');
    // }
  };
  ...
  const fetchConversations = async () => {
    try {
      ...
      console.log(`[CHAT DEBUG] Début récupération des conversations WhatsApp [${timestamp}]`);
      console.log(`[CHAT DEBUG] Client supabaseAdmin configuré:`, !!supabaseAdmin);
      
      // Afficher les conversations actuelles pour débogage
      if (conversations.length > 0) {
        console.log('[CHAT DEBUG] AVANT - État actuel des conversations:', conversations.map(c => ({
          id: c.id,
      ...
      console.log(`[CHAT DEBUG] Exécution de la requête Supabase avec filtrage par utilisateur...`);
      
      // Récupérer l'ID de l'utilisateur connecté depuis la session
      const { data: { session } } = await supabase.auth.getSession();
      
      if (!session?.user?.id) {
        console.log(`[CHAT DEBUG] Aucun utilisateur connecté, affichage de toutes les conversations`);
        setConversations([]);
        setLoading(false);
        return;
      }
      
      // Utiliser directement l'ID de session qui correspond à l'ID dans la table hosts
      const sessionUserId = session.user.id;
      const currentHostId = sessionUserId; // L'ID de session correspond à l'ID de la table hosts
      
      console.log(`[CHAT DEBUG] Session user ID: ${sessionUserId}`);
      console.log(`[CHAT DEBUG] Filtrage par host_id: ${currentHostId}`);
      ...
      console.log(`[CHAT DEBUG] Réponse de Supabase:`, { data, error, dataLength: data?.length });

      if (error) throw error;

      // Transformer les données pour compatibilité avec le composant
      const transformedData = data?.map(item => ({
        ...item,
        property: item.property_id ? [{ id: item.property_id, name: 'Propriété' }] : [{ id: null, name: 'Aucune propriété' }]
      })) || [];
