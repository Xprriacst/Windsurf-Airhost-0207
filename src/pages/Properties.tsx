...
- Pour les questions sur le check-in/check-out, réfère-toi aux dates de réservation.`;
  ...
  const loadProperties = async (retryCount = 0) => {
    ...
    try {
      console.log(`Chargement des propriétés depuis Supabase (tentative ${retryCount + 1})...`);
      setLoading(true);
      const data = await fetchProperties();
      console.log('Propriétés reçues de Supabase:', data);
      setProperties(data);
    } catch (error: any) {
      ...
      if (retryCount < maxRetries && (
        ...
        error.message?.includes('Impossible de charger')
      )) {
        console.log(`Nouvelle tentative dans 2 secondes... (${retryCount + 1}/${maxRetries})`);
        setTimeout(() => loadProperties(retryCount + 1), 2000);
        return;
      }
      
      // Afficher l'erreur seulement après épuisement des tentatives
      const errorMessage = error.message || 'Impossible de charger les propriétés';
  ...
  useEffect(() => {
    console.log('Properties useEffect déclenché - Chargement initial des propriétés');
    // Chargement initial des propriétés
    loadProperties();
    
    // Fonction pour recharger les propriétés quand l'utilisateur revient sur la page
    const handleFocus = () => {
      console.log('Fenêtre redevient active, rechargement des propriétés...');
      loadProperties();
    };
    
    // Fonction pour forcer le rechargement quand la page devient visible
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log('Page redevient visible, rechargement des propriétés...');
        loadProperties();
      }
    };
      ...
      {loading ? (
        ...
        <Box sx={{ textAlign: 'center', mt: 4 }}>
          ...
          <Typography variant="body2" color="textSecondary" sx={{ mt: 1, mb: 2 }}>
            Ajoutez votre premier appartement pour commencer à gérer vos conversations
          </Typography>
          <Button 
            variant="outlined" 
            color="primary" 
